...
public class CookieUtil {
     ...
     * @author jxf
     */
    public static List<Map<String,User>> getCookieAsList(String cookieValue){
    ...
    private static String cookieListToString(List<Map<String, User>> list) {
		String coolieString = "";
    	for (Map<String, User> map : list) {
			for (Entry<String, User> UserMap : map.entrySet()) {
				User User = UserMap.getValue();
				if(coolieString.isEmpty()) {
					coolieString = User.getUsername()+"#"+User.getPassword();
				}else {
					coolieString = coolieString+"@"+User.getUsername()+"#"+User.getPassword();
				}
			}
		}
    	return coolieString;
    ...
    public static HttpServletResponse setCookie(HttpServletResponse response, String name, String value,int time,HttpServletRequest request) {
    	...
    	try {
    		...
    		List<Map<String, User>> list = getCookieAsList(orgCookievalue);
    		for (Map<String, User> map : list) {
    			User orgUser = map.get(username);
